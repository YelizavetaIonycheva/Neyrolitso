...
public class DBUtils {
    ...
    public static void deleteDB(Context context) {
        File file = new File(context.getFilesDir() + DB_FILE_TT);
        if(file.exists()) {
            file.delete();
        }

        file = new File(context.getFilesDir() + DB_HISTORY_FILE_TT);
        if(file.exists()) {
            file.delete();
        }

        file = new File(context.getFilesDir() + DB_FILE_P);
        if(file.exists()) {
            file.delete();
        }

        file = new File(context.getFilesDir() + DB_HISTORY_FILE_P);
        if(file.exists()) {
            file.delete();
        }
    }
    ...
    public static SpoChatMessage [] getSpoChatMessagesRange(long idChatRoom, int startm, int endm, boolean isDescent) {
        ...
        for (int i = 0; i < messages.length; i++) {
            if (messages[i].getTypeContent() == SpoChatMessage.FILE) {
                messages[i].setSpoFiles(new ArrayList<>(Arrays.asList(getSpoFiles(messages[i].getId()))));
            }
        }
